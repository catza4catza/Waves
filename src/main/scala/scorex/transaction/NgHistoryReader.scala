...
class NgHistoryReader(ngState: () => Option[NgState], inner: History, fs: FunctionalitySettings)
  ...
  override def blockIdsAfter(parentSignature: ByteStr, howMany: Int): Seq[ByteStr] = ngState() match {
    case Some(ng) if ng.base.reference == parentSignature => Seq(ng.bestLiquidBlockId)
    case ng => inner.blockIdsAfter(parentSignature, howMany - ng.fold(0){ _ => 1 }) ++ ng.map(_.bestLiquidBlockId)
  }

  override def parent(block: Block, back: Int): Option[Block] = {
    ngState() match {
      case Some(ng) if ng.contains(block.reference) =>
        if (back == 1) Some(ng.bestLiquidBlock) else inner.parent(ng.base, back - 1)
      case _ => inner.parent(block, back)
    }
  }
