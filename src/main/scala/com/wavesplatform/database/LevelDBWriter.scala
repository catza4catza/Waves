  ...
  override def blockIdsAfter(parentSignature: ByteStr, howMany: Int): Seq[ByteStr] = readOnly { db =>
    db.get(k.heightOf(parentSignature)).fold(Seq.empty[ByteStr]) { parentHeight =>
      ((parentHeight + 1) to (parentHeight + howMany))
        .flatMap { h => db.get(k.blockHeader(h)) }
        .map { b => b._1.signerData.signature }
    }
  }

  override def parent(block: Block, back: Int): Option[Block] = readOnly { db =>
    db.get(k.heightOf(block.reference)).flatMap(h => db.get(k.blockAt(h - back)))
  }
